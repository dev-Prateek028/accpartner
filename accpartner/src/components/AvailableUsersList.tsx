...
function AvailableUsersList() {
  ...
  const sendPairingRequest = async (recipientId: string) => {
    ...
    if (!isWithinDeadline) {
      toast.error('Cannot send pairing requests after deadline');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {!isWithinDeadline ? (
        <div className="text-center py-8">
          <p className="text-yellow-800">Your deadline has passed. You can only verify tasks now.</p>
        </div>
          ...
          {availableUsers.map((availableUser) => (
            <li key={availableUser.id} className="py-4 flex items-center justify-between">
              ...
              <button
                ...
                disabled={requestingUserIds.includes(availableUser.id) || !isWithinDeadline}
